...
const MyProfieData = ({ cardData, handleDeleteService }) => {
    const { ratingData } = useContext(ShareContext)
    const [avgStars, setAvgStars] = useState();
    const [hover, setHover] = useState(0);


    const { img, title, price, details, _id, date, phote, userName } = cardData;





    const reviews = ratingData.filter(rating => rating.cardId === cardData._id)
            ...
            <div className="relative group h-[610px] border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden shadow-lg">
                ...
                <div className="my-6 py-3 px-6 flex flex-col justify-around">
                    ...
                    <div className='flex justify-between'>
                        ...
                        <button onClick={() => handleDeleteService(_id)} className=''>	<svg xmlns="http://www.w3.org/2000/svg" className="text-[#f96603] h-7 w-7 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
